...
function renderQuestion() {
  ...
  q.options.forEach((opt, i) => {
    ...
    div.type = 'button';
    div.className = 'option-card w-full text-left p-4 rounded-xl border-2 border-notary-200 transition-all duration-200 flex items-start gap-3';
    div.dataset.index = i;

    const letter = String.fromCharCode(65 + i);
    div.innerHTML = `
      <span class="flex-shrink-0 w-8 h-8 rounded-lg bg-notary-100 text-notary-700 font-bold flex items-center justify-center text-sm">${letter}</span>
      <span class="text-notary-800">${opt}</span>
    `;
  ...
  if (userAnswer !== undefined) {
    ...
    feedbackEl.className = 'mt-6 p-4 rounded-xl ' + (userAnswer.isCorrect ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' : 'bg-amber-50 text-amber-800 border border-amber-200');
